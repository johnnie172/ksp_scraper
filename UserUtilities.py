...
class UserUtilities:
    ...
    def user_login(self):
        ...
        user = self.dbq.select_user(email)
        id = user[0]
        email = user[1]
        password = user[2]
    ...
    def user_signup(self):
        ...
        user_id = self.dbq.add_user(email, password)
    ...
    def notify_out_of_stock(self, users_id_records, item_id):
        ...
        logger.debug(f'users id records are:{self.users_id_records}.')
        logger.debug(f'item_id is: {item_id}.')
        item_title = self.dbq.select_row(f'SELECT title FROM items WHERE id = {item_id}')[0]
        email_records = self.dbq.select_emails_to_notify(tuple(self.users_id_records))
    ...
    def notify_target_price(self, users_id_records):
        ...
        logger.debug(f'self.users_id_records are:{self.users_id_records}')

        users_list = []
        current_item_id = self.users_id_records[0][1]
        items_users_dict = {}

        for user in self.users_id_records:
        ...
        for item in items_users_dict:
            ...
            item_uin = self.dbq.select_row(f'SELECT uin FROM items WHERE id = {item}')
            current_users_ids = items_users_dict[item]
            logger.debug(f'current_users_ids is: {current_users_ids}')
            email_records = self.dbq.select_emails_to_notify(tuple(current_users_ids))
